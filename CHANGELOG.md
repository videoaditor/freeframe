...
### Changed
- **The n8n feedback and share-link database views are now migration-owned** - Alembic reproduces the existing automation contracts, adds stable comment revision and tombstone metadata, sends notifications for supported creates, updates, and soft deletions, and conditionally grants the existing `n8n_read` role read access to the views only.
